...
class FileHandler {
  ...
  generateTempWorkingDir() {
    ...
    var tmpDir = ospath.join(this.config.workingPath, guid);
            ...
            (stat) => {
              ...
              fsp.readFile(abspath).then(
                (contents) => {
                  ...
                  var newfile = new File({ path: posixify ? posixify(relpath) : relpath, contents, stat, src: { abspath } });
  ...
  #prepareSrc (newfile) {
    ...
    var oldHistory = ospath.join('modules/ROOT/attachements', newfile.history[0]);
  ...
  #loopSearchCopyComponentFiles(tmpDir, component, globPattern) {
    component.files.forEach(file => {
      if (minimatch(file.path, globPattern)) {
        // this.logger.warn(file.path);
        
        var newFile = ospath.join(tmpDir, file.path);
        // this.logger.warn(newFile);
        
        var parentDir = ospath.dirname(newFile);
        // this.logger.warn(parentDir);

        fs.mkdirSync(parentDir, {recursive: true});

        try {
          // if (fs.existsSync(newFile)) {
          //   this.logger.warn("file found: " + newFile);
          // } else {
          //   this.logger.warn("file not found: " + newFile);
          // }
          
          fs.unlinkSync(newFile)
          // this.logger.warn("file removed");
        } catch(err) {
          // this.logger.error(err)
        }
        fs.writeFileSync(newFile, file.contents.toString('utf-8'), {encoding:'utf8',flag:'w'});
        // fs.writeFile(newFile, file.contents.toString('utf-8'), (err) => {
